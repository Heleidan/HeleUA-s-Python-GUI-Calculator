...
class Calculator:
    def __init__(self):
        ...
        #icon
        self.window.iconbitmap(default='main_icon.ico')
        #Display Label 100%
        ...
        self.bind_keys()
    #to use computer keyboard numbers
    def bind_keys(self):
    ...
    def create_operation_buttons(self):
        ...
        for operator, symbol in self.operations.items():
            button = tk.Button(self.buttons_frame, text=symbol, bg=OFF_WHITE, fg=LABEL_COLOR, font=DEFAULT_FONT_STYLE, borderwidth=0, command=lambda y=operator: self.append_operator(y))
    ...
    def create_clear_button(self):
        button = tk.Button(self.buttons_frame, text="CE", bg=OFF_WHITE, fg=LABEL_COLOR, font=DEFAULT_FONT_STYLE, borderwidth=0, command=self.clear)
    ...
    def update_total_label(self):
        ...
        for operator, synbol in self.operations.items():
            expression = expression.replace(operator, f' {synbol} ') #replace symbols in display_frame
        self.total_label.config(text=expression)

    def update_label(self):
        self.label.config(text=self.current_expression[:11]) #prevent string overflow
